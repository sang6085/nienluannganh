...
const DocumentShare: React.FC = () => {
	...
	const shareFolder = async (item: any) => {
		setAnchorEl(null);
		setTitleDialog('share');

		const response = await ShareToGet(item.id);
		if (response) {
			if (response.errorCode === null) {
				console.log(response.data);
				setValue({ id: item.id, folderName: item.name, name: response.data });
				setOpenDialog(true);
			}
		}
	...
	const closeDialog: (result: boolean) => void = (result) => {
		if (result) {
			setOpenDialog(false);
		}
	};
			...
			{dataFolder.map((item: any, index: number) => (
				<Grid item xs={3} key={index}>
					...
					>
						...
						<MenuItem onClick={() => shareFolder(item)}>
							<PersonAddAlt1Icon />
							&nbsp;&nbsp;&nbsp; {t('tenant.share')}
						</MenuItem>
					</Menu>
				...
				<DialogContent>
					{showCreateFolder ? (
						<CreateDocument
							flag={flag}
							value={value}
							flagRenameFile={flagRenameFile}
							closeDialog={closeDialog}
						/>
					) : (
						<ShareFolder value={value} closeDialog={closeDialog} flag={flag} />
					)}
				</DialogContent>
			</Dialog>
			<Dialog
